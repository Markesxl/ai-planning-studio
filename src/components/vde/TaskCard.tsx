...
import { DeleteConfirmDialog } from "@/components/vde/DeleteConfirmDialog";
...
export const TaskCard = forwardRef<HTMLDivElement, TaskCardProps>(
  ({ task, index, onToggle, onDelete, onClick, categoryColor }, ref) => {
    ...
    const handleClick = (e: React.MouseEvent) => {
      ...
      if (target.closest('button') || target.closest('[role="checkbox"]') || target.closest('[role="alertdialog"]')) {
        return;
      }
      onClick?.(task);
    };
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 min-w-0 space-y-1.5 md:space-y-2">
          ...
          <p className={cn("text-xs md:text-sm leading-relaxed transition-all duration-200", task.done && "line-through opacity-50")}>
        ...
        >
          <DeleteConfirmDialog
            ...
            description="Essa ação não pode ser desfeita. A tarefa será removida permanentemente."
